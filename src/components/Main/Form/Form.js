...
const Form = () => {
    ...
    useEffect(()=>{
        if(segment){
            if(segment.intent.intent==="add_expense"){
                ...
                return setFormData(intitialState);
            }
            ...
            segment.entities.forEach((e)=>{
                ...
                switch (e.type) {
                    ...
                    case 'category':
                        ...
                        setFormData({...formatDate,type:'Income',category});
                        }else if(expenseCategories.map((eC)=>eC.type).includes(category)){
                            setFormData({...formatDate,type:'Expense',category});
                            }
                        break;
                    case 'date':
                        setFormData({...formatDate,date:e.value});
                        break;
    ...
    return ( 
        <Grid container spacing={2}>
            ...
            <Button className={classes.button} variant="outlined" onClick={createTransaction}>Create</Button>
        </Grid>
     );
}
